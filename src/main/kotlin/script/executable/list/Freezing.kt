...
@ExecutionSign
object Freezing : ExecutionParser {
    override val headRegex = Regex("(?i)freeze|freezing")

    override fun parse(content: Any?): Execution {
        if (content == null) throw ParseException("freezing的内容不可为空")
        val s = content.toString()
        return when {
            s.startsWith("+") -> Execution { p: Player ->
                try {
                    val max = p.maxFreezeTicks
